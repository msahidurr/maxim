...
<table class="table table-bordered">
    ...
    <tbody>
    	<?php
    		...
    		$BDTandUSD = 0;
    	 ?>
    	 	@foreach ($orderValue as $counts)
    	 		<?php
    	 			$count = 1;
    	 			$rowspanValue += $count ; 
    	 		 ?>
    	 	@endforeach
    		@foreach ($orderValue as $item)
    			...
    			?>
	    			...
	    			<tr>
	    				...
	    				<!-- @for($ik; $ik<=0; $ik++) -->
	    				<!-- @endfor -->
	    				<td rowspan="">{{isset($item->oss)?$item->oss:''}}</td>
	    				
			    		<td rowspan="{{$itemlength}}">{{isset($item->style)?$item->style:''}}</td> 
